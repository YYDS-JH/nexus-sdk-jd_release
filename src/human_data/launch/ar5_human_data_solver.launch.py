...
def _resolve_node_name(robot_id: str, base_name: str) -> str:
    """Avoid ar5 + ar5_human_data_solver → ar5_ar5_human_data_solver."""
    if not robot_id:
        return base_name
    prefix = f'{robot_id}_'
...
def launch_setup(context, *args, **kwargs):
    ...
    base_name = 'ar5_human_data_solver'
    node_name = _resolve_node_name(robot_id, base_name)
    config_path = LaunchConfiguration('ar5_human_data_config').perform(context)
...
def generate_launch_description():
    ...
    default_config = os.path.join(
        pkg_share, 'config', 'ar5_human_data_config.yaml')

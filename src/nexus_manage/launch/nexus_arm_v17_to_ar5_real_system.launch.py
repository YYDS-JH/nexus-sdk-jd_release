#!/usr/bin/env python3
"""
主启动文件 - 一键启动整个远程操控系统（Nexus-Arm V17 → AR5）

用法:
  # 全部启动（默认，domain_id=0）
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py

  # 仅启动 Slave 侧（AR5 机器上运行）
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=slave

  # 仅启动 Master 侧（Nexus-Arm 机器上运行）
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=master

  # 指定域 ID（无需修改 ~/.bashrc）
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=slave domain_id:=42

  # 指定 DDS 通信网卡（多网卡机器必须显式指定，否则可能选错网卡导致跨机发现失败）
  # 查看可用网卡：ip -4 addr | grep "10.18"
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=master network_interface:=enp3s0
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=slave  network_interface:=eth0
  # 也可以直接填本机 IP（与网卡名等效）
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=master network_interface:=10.18.64.46
  ros2 launch nexus_manage nexus_arm_v17_to_ar5_real_system.launch.py role:=slave  network_interface:=10.18.20.25
...
def launch_setup(context, *args, **kwargs):
    ...
    nexus_arm_human_data_solver_launch = _include('human_data', 'nexus_arm_v17_human_data_solver.launch.py', launch_args)
    master_controller_launch = _include('robot_controller', 'master_single_nexus.launch.py', launch_args)
    manager_launch = _include('nexus_manage', 'nexus_nexus-arm_v17_to_ar5_manage.launch.py', launch_args)

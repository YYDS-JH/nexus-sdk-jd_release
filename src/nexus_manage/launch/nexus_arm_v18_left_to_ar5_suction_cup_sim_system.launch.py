#!/usr/bin/env python3
"""
主启动文件 - Nexus-Arm V18 Left 控制 AR5 Suction Cup 仿真系统
"""
...
def generate_launch_description():
    ...
    # 2. Nexus-Arm V18 Left 仿真器 Launch
    nexus_arm_sim_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('mujoco_sim'),
                'launch',
                'nexus_arm_v18_left_sim.launch.py'
            ])
        ])
    )
    ...
    manager_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                ...
                'launch',
                'nexus_nexus-arm_v18_to_ar5_suction_cup_manage.launch.py'
            ])
        ])
    )

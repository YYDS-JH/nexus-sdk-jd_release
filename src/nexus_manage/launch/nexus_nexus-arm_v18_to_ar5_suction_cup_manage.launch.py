"""
Teleop Manager Node Launch File - Nexus-Arm V18 to AR5 Suction Cup
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    await_scheduler = LaunchConfiguration(
        'boot_selfcheck_await_scheduler').perform(context).strip().lower()
    base_name = 'nexus_arm_to_ar5_suction_cup_manager_node'
    ...
    params = [
        config_path,
        {'boot_selfcheck_await_scheduler': await_scheduler != 'false'},
    ]
    if robot_id:
...
def generate_launch_description():
    ...
    default_config = os.path.join(
        pkg_share, 'config',
        'nexus-arm_v18_to_ar5_suction_cup_manage.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'boot_selfcheck_await_scheduler',
            default_value='true',
            description=(
                'When false, skip Phase 1 scheduler wait and use robot_name '
                'from config (legacy single-slave cross-subnet mode)'
            ),
        ),

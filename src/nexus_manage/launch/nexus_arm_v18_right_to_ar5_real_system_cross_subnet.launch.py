...
def _compose_launch_actions(node_specs):
    regular_actions = []
    delayed_manage_action = None

    for pkg, launch_file in node_specs:
        action = _include(pkg, launch_file)
        if pkg == 'nexus_manage':
...
def launch_setup(context, *args, **kwargs):
    ...
    # 组装节点
    if role == 'master':
        launch_actions = _compose_launch_actions(_MASTER_NODES)
    elif role == 'slave':
        launch_actions = _compose_launch_actions(_SLAVE_NODES)
    else:
        launch_actions = _compose_launch_actions(_MASTER_NODES + _SLAVE_NODES)

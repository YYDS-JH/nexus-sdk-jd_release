...
用法:
  ...
  # === Slave 侧（supervisor 监控，默认）===
  ros2 launch nexus_manage nexus_arm_v18_right_to_ar5_suction_cup_real_system_cross_subnet.launch.py role:=slave robot_id:=ar5
  # 或仅启动 supervisor 常驻进程：
  ros2 launch nexus_manage slave_stack_supervisor_suction_cup.launch.py robot_id:=ar5

  # === Slave 侧（无 supervisor，全栈常开）===
  ros2 launch nexus_manage nexus_arm_v18_right_to_ar5_suction_cup_real_system_cross_subnet.launch.py role:=slave supervisor_mode:=false robot_id:=ar5

  # === 多从臂（registry + robot_id + scheduler car 三者一致）===
  ros2 launch nexus_manage nexus_arm_v18_right_to_ar5_suction_cup_real_system_cross_subnet.launch.py role:=slave robot_id:=ar5_01

端口规则 (ParticipantIndex=auto, DomainGain=0):
  Participant port = _PORT_BASE + ParticipantIndex
  stack 占 7000-7002 (auto)，supervisor 固定 7003。
"""
...
_STACK_LAUNCH_FILE = 'slave_stack_only_suction_cup.launch.py'
...
def _build_legacy_dds_configs(role, supervisor_mode, stack_only,
    ...
    if use_supervisor:
        ...
        print(
            f'[cross_subnet/suction_cup] stack ports {_PORT_BASE}-'
            f'{_PORT_BASE + len(_SLAVE_NODES) - 1} (auto), supervisor fixed port '
            f'{_PORT_BASE + _SUPERVISOR_PARTICIPANT_INDEX} '
            f'(ParticipantIndex={_SUPERVISOR_PARTICIPANT_INDEX})'
        )
    else:
        print(
            f'[cross_subnet/suction_cup] DDS peers: local={local_count} '
...
def launch_setup(context, *args, **kwargs):
    ...
    if stack_dds_path:
        print(f'[cross_subnet/suction_cup] 复用 stack DDS 配置: {stack_dds_path}')
        if use_supervisor:
            if supervisor_dds_path:
                print(f'[cross_subnet/suction_cup] 复用 supervisor DDS 配置: {supervisor_dds_path}')
            else:
                print('[cross_subnet/suction_cup] 未找到 supervisor DDS 配置，将重新生成 (port 7003)')
    ...
    if not stack_dds_path:
        ...
        supervisor_dds_path = _write_cyclonedds_config(
            supervisor_xml, _DDS_SUPERVISOR_CONFIG_PREFIX)
        print(
            f'[cross_subnet/suction_cup] 补建 supervisor DDS (port '

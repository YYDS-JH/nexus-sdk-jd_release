#!/usr/bin/env python3
"""独立启动 slave_stack_supervisor（从臂 Orin 常驻，吸盘栈）。"""
...
def _launch_setup(context, *args, **kwargs):
    ...
    cmd = resolve_supervisor_cmd(
        robot_id,
        stack_launch_file='slave_stack_only_suction_cup.launch.py',
    )
    cmd.extend([
        '-p', f'yj_device_id:={yj_device_id}',
        '-p', f'yj_target_ip:={yj_target_ip}',
        '-p', f'auto_start_on_boot:={auto_start}',
        '-p', 'stack_launch_file:=slave_stack_only_suction_cup.launch.py',
    ])
